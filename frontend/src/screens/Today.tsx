...
export function Today() {
  ...
  function countdown(dateIso: string) {
    ...
    if (diffDays === 0) return t('today.dueToday');
    if (diffDays > 0) return t('today.countdownFuture', { days: diffDays });
    return t('today.countdownPast', { days: -diffDays });
  }
  ...
  const days = useMemo(() => {
    const out: { date: string; destinationId: string; cityLabel: string }[] = [];
    for (const d of destinations) {
      let cur = new Date(d.startDate);
      const end = new Date(d.endDate);
      while (cur <= end) {
        out.push({ date: cur.toISOString().slice(0, 10), destinationId: d.id, cityLabel: displayName(d) });
    ...
    <div style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: '6px 0 calc(102px + env(safe-area-inset-bottom))' }}>
      <div style={{ padding: '12px 22px 14px' }}>
        ...
        <div style={{ font: "600 30px/1.15 'Noto Sans Hebrew',sans-serif", letterSpacing: '-.5px', marginTop: 8 }}>
          {dest ? displayName(dest) : t('today.noDestination')}
        </div>
        <div style={{ font: "400 12.5px/1.4 'Noto Sans Hebrew',sans-serif", color: 'var(--text-dim)', marginTop: 7 }}>
          {scheduled.length === 0 ? t('today.noneScheduled') : scheduled.length === 1 ? t('today.oneScheduled') : t('today.manyScheduled', { count: scheduled.length })}
        </div>
      </div>
      ...
      <div ref={railRef} style={{ display: 'flex', gap: 8, overflow: 'auto', padding: '0 22px 18px' }}>
        {days.map((d) => (
          <div key={d.date} data-day={d.date} onClick={() => setDate(d.date)}
            ...
            <div style={{ font: "500 10px 'Noto Sans Hebrew',sans-serif", color: date === d.date ? 'var(--bg)' : 'var(--text-dim)', whiteSpace: 'nowrap' }}>{d.cityLabel}</div>
      ...
      {scheduled.length > 0 && cityDest ? (
        <div style={{ padding: '0 22px' }}>
          <div className="card" style={{ background: cityCardBg(cityDest.colorKey, dark, palette) }}>
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ font: "400 11.5px 'Noto Sans Hebrew',sans-serif", color: 'var(--text-dim)', letterSpacing: '.6px' }}>{first.hour || t('today.firstOfDay')}</div>
              <div style={{ font: "600 21px/1.3 'Noto Sans Hebrew',sans-serif", marginTop: 8 }}>{displayName(first)}</div>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 14, flexWrap: 'wrap' }}>
                ...
                <span className="pill" style={{ border: `1px solid ${TAGS[first.tag]?.color}`, background: 'transparent', color: TAGS[first.tag]?.color }}>{tagLabel(first.tag, t)}</span>
                <span onClick={() => setMoving(first)} className="pill" style={{ cursor: 'pointer', fontWeight: 600, border: '1px solid var(--border)' }}>{t('today.postpone')}</span>
              </div>
            </div>
          </div>
            ...
            <div key={s.id} style={{ display: 'flex', gap: 14, padding: '14px 0', borderTop: '1px solid var(--border-soft)', opacity: s.myStatus === 'skipped' ? 0.5 : 1 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: 7, marginTop: 9, flexWrap: 'wrap' }}>
                  ...
                  <span onClick={() => setMoving(s)} className="pill" style={{ cursor: 'pointer', border: '1px solid var(--border)' }}>{t('today.postpone')}</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ margin: '0 22px', border: '1px dashed var(--border)', borderRadius: 20, padding: '24px 20px', textAlign: 'center' }}>
          <div style={{ font: "600 16px 'Noto Sans Hebrew',sans-serif" }}>{t('today.emptyTitle')}</div>
          {dest ? (
            <>
              <div style={{ font: "400 12.5px/1.55 'Noto Sans Hebrew',sans-serif", color: 'var(--text-dim)', marginTop: 8 }}>
                {t('today.emptyWithDestDesc', { date: dayLabel(date).dm, city: displayName(dest) })}
              </div>
              <div className="btn btn-accent" style={{ marginTop: 16, display: 'inline-block' }} onClick={() => navigate(`/city/${dest.id}`)}>{t('today.openCity', { city: displayName(dest) })}</div>
            </>
          ) : (
            <>
              <div style={{ font: "400 12.5px/1.55 'Noto Sans Hebrew',sans-serif", color: 'var(--text-dim)', marginTop: 8 }}>
                {t('today.emptyNoDestDesc')}
              </div>
              <div className="btn btn-accent" style={{ marginTop: 16, display: 'inline-block' }} onClick={() => navigate('/trip')}>{t('today.goToTrip')}</div>
            </>
          )}
        </div>
      )}
      ...
      {unscheduled.length > 0 && (
        <div style={{ padding: '26px 22px 0' }}>
          <div className="section-label" style={{ paddingBottom: 6 }}>{dest ? t('today.unscheduledHeaderCity', { city: displayName(dest), count: unscheduled.length }) : t('today.unscheduledHeader', { count: unscheduled.length })}</div>
          {unscheduled.map((s: any) => (
            <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, padding: '13px 0', borderTop: '1px solid var(--border-soft)' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ font: "500 14px/1.3 'Noto Sans Hebrew',sans-serif" }}>{displayName(s)}</div>
                <div style={{ font: "400 11.5px 'Noto Sans Hebrew',sans-serif", color: 'var(--text-dim)', marginTop: 3 }}>{[s.duration, tagLabel(s.tag, t)].filter(Boolean).join(' · ')}</div>
              </div>
              <div className="pill" style={{ cursor: 'pointer', fontWeight: 600, color: 'var(--accent)', border: '1px solid var(--border)' }} onClick={() => assignToday(s.id)}>{t('today.assignToday')}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <Drawer open={!!moving} onClose={() => setMoving(null)}>
        {moving && (
          <>
            <div style={{ font: "400 11px 'Noto Sans Hebrew',sans-serif", color: 'var(--text-dim)', letterSpacing: '.6px' }}>{t('today.postponeTitle')}</div>
            <div style={{ font: "600 20px/1.25 'Noto Sans Hebrew',sans-serif", marginTop: 7 }}>{displayName(moving)}</div>
            <div style={{ font: "400 12.5px/1.45 'Noto Sans Hebrew',sans-serif", color: 'var(--text-dim)', marginTop: 6 }}>
              {t('today.postponeDesc', { day: moving.day ? dayLabel(moving.day).dm : '—' })}
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '26px 0 0', borderTop: '1px solid var(--border-soft)', marginTop: 22 }}>
              <div onClick={() => setMoving(null)} style={{ font: "500 13px 'Noto Sans Hebrew',sans-serif", color: 'var(--text-dim)', cursor: 'pointer' }}>{t('common.cancel')}</div>
              <div onClick={() => reschedule(moving.id, '')} style={{ font: "600 13px 'Noto Sans Hebrew',sans-serif", color: 'var(--danger)', cursor: 'pointer' }}>{t('today.removeDate')}</div>
            </div>
          </>
        )}
      </Drawer>
