...
export function JoinTrip() {
  ...
  async function submit() {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.message : t('join.genericError'));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="app-shell" style={{ justifyContent: 'center', padding: '0 26px 40px' }}>
      <div onClick={() => navigate('/choose')} style={{ font: "500 13px 'Noto Sans Hebrew',sans-serif", color: 'var(--accent)', cursor: 'pointer', padding: '8px 0' }}>{t('common.back')}</div>
      <div style={{ font: "600 27px/1.25 'Noto Sans Hebrew',sans-serif", letterSpacing: '-.4px', marginTop: 20 }}>{t('join.title')}</div>
      <div style={{ font: "400 13.5px/1.6 'Noto Sans Hebrew',sans-serif", color: 'var(--text-dim)', marginTop: 10 }}>
        {t('join.desc')}
      </div>
      <div style={{ marginTop: 26 }}>
        <div className="section-label" style={{ paddingBottom: 9 }}>{t('join.codeLabel')}</div>
      ...
      {preview && (
        <div style={{ marginTop: 16, border: '1px solid var(--border)', borderRadius: 16, padding: 16 }}>
          <div style={{ font: "400 11px 'Noto Sans Hebrew',sans-serif", color: 'var(--text-dim)', letterSpacing: '.5px' }}>{t('join.found')}</div>
          ...
          <div style={{ font: "400 12.5px/1.5 'Noto Sans Hebrew',sans-serif", color: 'var(--text-dim)', marginTop: 5 }}>
            {t('join.previewLine', { destinations: preview.destinations, members: preview.members, owner: preview.ownerName })}
          </div>
        </div>
      )}
